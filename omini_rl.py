...
from verl.utils.hithink_utils import load_file, normalize_sample_schema, normalize_dataset_features
...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        for parquet_file in self.data_files:
            ...
            if parquet_file.endswith(".parquet"):
                ...
                dataframe = dataframe.map(normalize_sample_schema)
                dataframes.append(dataframe)
                data_count[parquet_file] = len(dataframe)
            elif parquet_file.endswith(".json") or parquet_file.endswith(".jsonl"):
                for k, d in load_file(parquet_file, seed=self.config.get('seed', 1)):
                    dataframes.append(d)
                    data_count[k] = len(d)
            else:
                raise ValueError(f"Unsupported file format: {parquet_file}")

        dataframes = normalize_dataset_features(dataframes)
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        if image_key in doc and doc[image_key]:
                            images = [
                                process_image(image, image_patch_size=self.processor.image_processor.patch_size if hasattr(self.processor, 'image_processor') else self.image_patch_size) for image in doc[image_key]
                            ]
                        else:
                            images = None
                        ...
                        if video_key in doc and doc[video_key]:
                            videos, video_metadata = zip(
                                *[
                                    process_video(
                                        video, image_patch_size=self.processor.image_processor.patch_size if hasattr(self.processor, 'image_processor') else self.image_patch_size, return_video_metadata=True
                                    )
                        ...
                        processor_kwargs = dict(text=[raw_prompt], images=images, videos=videos, videos_kwargs=videos_kwargs)
                        audios = []
                        if self.use_audio_in_video and video_key in doc and doc[video_key]:
                            audios.extend(process_audio(v, is_video=True) for v in doc[video_key])
                        if self.audio_key in doc and doc[self.audio_key]:
                            audios.extend(process_audio(a, is_video=False) for a in doc[self.audio_key])
                        if audios:
                            processor_kwargs["audios"] = audios
                            # Some processors (e.g. Qwen-Omni) expose the kwarg as singular `audio`.
                            processor_params = inspect.signature(processor).parameters
                            if "audios" not in processor_params and "audio" in processor_params:
                                processor_kwargs["audio"] = processor_kwargs.pop("audios")
    ...
    def _build_messages(self, example: dict):
        """Replace <image>, <video> and <audio> placeholder in messages with corresponding modality
        entries which is required by processor.apply_chat_template.
        ...
        no_mm = not (images or videos or audios)
        for m in messages:
            if isinstance(m['content'], list):
                if not no_mm:  # 如果images/videos/audios非空，说明是parquet（verl原始格式），content类型为text
                    m['content'] = m['content'][0]['text']
                    continue
                for c in m['content']:
                    for k in list(c):
                        if c[k] is None:  # 过滤值为None的字段，防止image_url: None导致报错
                            del c[k]
        ...
        for message in messages:
            if not images and not videos and not audios:
                continue
            assert self.processor is not None, "processor is needed to process image/video/audio"
    ...
    def __getitem__(self, item):
        ...
        row_dict["extra_info"].setdefault("messages", row_dict["raw_prompt"])
        return row_dict
    ...
    async def process_vision_info(
        ...
        image_patch_size,
        config: DictConfig,
    ) -> tuple[list[Image.Image], list[tuple[torch.Tensor, dict]], list]:
        ...
        >>> images, videos, audios = RLHFDataset.process_vision_info(messages, image_patch_size)
        ...
        ...                    video_metadata=video_metadatas, do_sample_frames=False, audio=audios)
        ...
        use_audio_in_video = bool(config.get("use_audio_in_video", False)) if config is not None else False
        # process_audio_info dispatches strictly on ele["type"] in ("audio", "video"), but dataset
        # content often carries "audio_url"/"video_url" (as qwen_vl_utils' vision path tolerates).
        # Map those to the bare modality name on a shallow copy so audio entries are picked up.
        type_map = {"audio_url": "audio", "video_url": "video"}
        for m in messages:
            if isinstance(m.get("content"), list):
                for i, c in enumerate(m["content"]):
                    if isinstance(c, dict) and (t := type_map.get(c["type"])) is not None:
                        m["content"][i] = c.copy()
                        m["content"][i]["type"] = t
        audios = process_audio_info(messages, use_audio_in_video=use_audio_in_video)
