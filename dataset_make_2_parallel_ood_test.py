...
def build_rl_dataset(
    ...
    with open(jsonl_path, 'w', encoding='utf-8') as fout:
        for idx, row in df.iterrows():
            ...
            stock_code = extract_stock_code(image_path_str)
            print(stock_code)
            if not stock_code:
                print(f"[skip] row {idx}: cannot extract stock code from '{image_path_str}'")
                skipped += 1
                continue

            # image_path_str is now a full https:// URL; fall back to local path if needed
            if image_path_str.startswith('http'):
                img_b64 = load_image_as_pil(image_path_str)
                if img_b64 is None:
                ...
                img_b64 = load_image_as_pil(image_full_path)
                if img_b64 is None:
