...
def _balance_truncated_json(text: str) -> str:
    """Repair the common case where generation truncates final JSON closers.

    This is intentionally conservative: it only appends missing `}` / `]`
    based on bracket balance outside strings. It does not rewrite keys, values,
    commas, quotes, or arbitrary malformed content.
    """
...
def _loads_tool_call_json(text: str) -> dict[str, Any]:
    try:
        return json.loads(text)
    except json.JSONDecodeError:
        repaired = _balance_truncated_json(text)
...
class HermesToolParser(ToolParser):
    ...
    async def extract_tool_calls(
        ...
        for match in matches:
            try:
                function_call = _loads_tool_call_json(match)
...
@ToolParser.register("tool_begin")
class ToolBeginParser(ToolParser):
    """
    Tool parser for outputs like:
    <tool_begin>{"name": "FinQuery", "input": "..."}</tool_end>

    The emitted JSON follows the agent-o3 planning format, while verl tools
    expect FunctionCall(name=..., arguments=<JSON object matching tool schema>).
    """

    def __init__(self, tokenizer) -> None:
        super().__init__(tokenizer)

        self.tool_call_start_token: str = "<tool_begin>"
        self.tool_call_end_token: str = "</tool_end>"
        self.tool_call_regex = regex.compile(r"<tool_begin>(.*?)</tool_end>", regex.DOTALL)

    def _get_tool_parameters(self, tool_name: str, tools: Optional[list[OpenAIFunctionToolSchema]]) -> dict:
        if not tools:
            return {}

        for tool in tools:
            if tool.type == "function" and tool.function.name == tool_name:
                return {k: v.model_dump() for k, v in tool.function.parameters.properties.items()}
        return {}

    def _build_arguments(self, tool_call: dict, tools: Optional[list[OpenAIFunctionToolSchema]]) -> dict:
        ...
        parameters = self._get_tool_parameters(tool_name, tools)
        tool_input = arguments.pop("input")
        ...
        if "input" in parameters:
            ...
            try:
                ...
                if isinstance(parsed_input, dict):
                    arguments.update(parsed_input)
                else:
                    arguments["input"] = parsed_input
        else:
            arguments["input"] = tool_input
    ...
    async def extract_tool_calls(
        ...
        for match in self.tool_call_regex.findall(text):
            try:
                ...
                name = tool_call["name"]
                arguments = self._build_arguments(tool_call, tools)
                function_calls.append(FunctionCall(name=name, arguments=json.dumps(arguments, ensure_ascii=False)))
            except Exception as e:
                logger.error(f"Failed to decode tool call: {e}")

        content = text[: text.find(self.tool_call_start_token)]
