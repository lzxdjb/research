...
class ActorRolloutRefWorker(MegatronWorker, DistProfilerExtension):
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @GPUMemoryLogger(role="update_actor_seeupo", logger=logger)
    @DistProfiler.annotate(color="red", role="actor_update_seeupo")
    def update_actor_seeupo(self, data: DataProto):

        """
        SeeUPO version of update_actor. Identical wrapper except it calls
        update_policy_seeupo instead of update_policy.
        """
        assert self._is_actor
    
        if self._is_offload_param:
            load_megatron_model_to_gpu(self.actor_module)
            log_gpu_memory_usage("After load actor params during update_actor_seeupo", logger=logger)
        if self._is_offload_optimizer:
            load_megatron_optimizer(self.actor_optimizer)
            log_gpu_memory_usage("After load optimizer during update_actor_seeupo", logger=logger)
    
        micro_batch_size = self.config.actor.ppo_micro_batch_size_per_gpu
        data.meta_info["micro_batch_size"] = micro_batch_size
        dataloader = self.actor.make_minibatch_iterator(data=data)
    
        with Timer(name="update_policy_seeupo", logger=None) as timer:
            metrics = self.actor.update_policy_seeupo(dataloader=dataloader)
        ...
        # Multiply by num_turns instead of ppo_epochs to account for T optimizer steps
        turn_index  = data.batch.get("turn_index", None)
        num_turns   = (
            int(turn_index[turn_index >= 0].max().item()) + 1
            if turn_index is not None else 1
        )
        metrics["perf/mfu/actor"] = (
            estimated_flops * num_turns / promised_flops / self.world_size
        )
        metrics["perf/max_memory_allocated_gb"] = get_torch_device().max_memory_allocated() / (1024**3)
        metrics["perf/max_memory_reserved_gb"]  = get_torch_device().max_memory_reserved() / (1024**3)
        metrics["perf/cpu_memory_used_gb"]      = psutil.virtual_memory().used / (1024**3)
        ...
        if self._is_offload_param:
            offload_megatron_model_to_cpu(self.actor_module)
            log_gpu_memory_usage("After offload actor params during update_actor_seeupo", logger=logger)
        if self._is_offload_optimizer:
            offload_megatron_optimizer(self.actor_optimizer)
            log_gpu_memory_usage("After offload optimizer during update_actor_seeupo", logger=logger)
    
        aggressive_empty_cache(force_sync=True)
        return output
    
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @GPUMemoryLogger(role="update_actor_distill", logger=logger)
    @DistProfiler.annotate(color="red", role="update_actor_distill")
    def update_actor_distill(self, data: DataProto):
        """
        ERL internalization step: supervised distillation on (x → y2).
        Uses SFT cross-entropy loss (importance ratio = 1, no clipping).
        """
        assert self._is_actor
        if self._is_offload_param:
            load_megatron_model_to_gpu(self.actor_module)
        ...
        data.meta_info["erl_distill"] = True   # signal to loss_func

        # Use a single epoch for distillation (no need for multiple passes)
        dataloader = self.actor.make_distill_minibatch_iterator(data=data)
        ...
        for batch in dataloader:
            ...
            for metric in metric_micro_batch["output"]:
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with adapter_ctx:
            output, entropys, layers_topk_idx, sum_pi_squared = self.actor.compute_log_prob(
