...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def update_critic(self, data: DataProto):
        ...
        for optional_key in ("turn_index", "old_log_probs"):
            if optional_key in data.batch:
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    value_loss_weights, weight_metrics = core_algos.build_value_loss_weights(
                        response_mask=response_mask,
                        mode=getattr(self.config, "value_loss_weight_mode", "none"),
                        turn_index=model_inputs.get("turn_index", None),
                        old_log_probs=model_inputs.get("old_log_probs", None),
                        normalize=getattr(self.config, "value_loss_weight_normalize", True),
                        clip_min=getattr(self.config, "value_loss_weight_clip_min", None),
                        clip_max=getattr(self.config, "value_loss_weight_clip_max", None),
                        clip_renormalize=getattr(self.config, "value_loss_weight_clip_renormalize", True),
                        rho=getattr(self.config, "value_loss_weight_rho", 1.0),
                        alpha=getattr(self.config, "value_loss_weight_alpha", 1.0),
                    )
