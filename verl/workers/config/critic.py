...
class CriticConfig(BaseConfig):
    ...
    Args:
        ...
        value_loss_weight_mode (str): Optional value-loss weighting mode for critic/baseline training.
            Options include none, token_score_norm, turn_score_norm, suffix_score_norm, turn_suffix_score_norm.
        value_loss_weight_normalize (bool): Normalize value-loss weights to mean 1 over valid tokens.
        value_loss_weight_clip_min (Optional[float]): Lower clamp for normalized value-loss weights.
        value_loss_weight_clip_max (Optional[float]): Upper clamp for normalized value-loss weights.
        value_loss_weight_clip_renormalize (bool): Re-normalize clipped weights back to mean 1.
        value_loss_weight_rho (float): Discount factor for suffix-energy critic weights.
        value_loss_weight_alpha (float): Exponent applied to normalized suffix-energy critic weights.
    ...
    value_loss_weight_mode: str = "none"
    value_loss_weight_normalize: bool = True
    value_loss_weight_clip_min: Optional[float] = None
    value_loss_weight_clip_max: Optional[float] = None
    value_loss_weight_clip_renormalize: bool = True
    value_loss_weight_rho: float = 1.0
    value_loss_weight_alpha: float = 1.0
