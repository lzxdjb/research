...
@dataclass
class MemoryBankConfig(BaseConfig):
    save_dir: str = "/tmp/hotpot_memory"
    max_entries: int = 100
    prune_threshold: int = -1
    dedup_threshold: float = 0.7    # Jaccard similarity above this → skip add
    top_k: int = 5
    reflect_prompt_path: Optional[str] = None
    reflect_temperature: float = 0.3
    reflect_max_tokens: int = 512


@dataclass
class PAGConfig(BaseConfig):
    max_attempts: int = 3
    answer_reward_weight: float = 1.0
    verifier_reward_weight: float = 1.0
...
class MultiTurnConfig(BaseConfig):
    ...
    tool_config_paths: Optional[dict[str, str]] = None
    ...
    use_seeupo: bool = False
    erl_reward_threshold: float = 0.5   # reflect if first attempt score < this
    erl_always_reflect: bool = False    # set true to always reflect (ablation)
    erl_memory_max_size: int = 64      # ring buffer capacity
    erl_memory_max_inject: int = 3     # max historical reflections injected
    memory: MemoryBankConfig = field(default_factory=MemoryBankConfig)
    pag: PAGConfig = field(default_factory=PAGConfig)

    # Wikipedia QA user-simulator reflection loop. These are consumed by
    # WikiUserSimReflectAgentLoop and ignored by other agent loops.
    wiki_user_sim_max_rounds: int = 3
    wiki_user_sim_system_prompt_path: Optional[str] = None
    wiki_user_sim_user_prompt_path: Optional[str] = None
    wiki_user_sim_temperature: float = 0.7
    wiki_user_sim_top_p: float = 0.95
    wiki_user_sim_max_tokens: int = 256
    wiki_user_sim_solver_max_tokens_per_turn: int = 0
    wiki_user_sim_answer_max_chars: int = 4000

    # HDL generate/judge/revise loop. Consumed by HDLAgentLoop and ignored by
    # generic tool/user-simulator loops.
    hdl_max_rounds: int = 2
    hdl_pass_score_threshold: float = 0.999
    hdl_judge_timeout: int = 30
    hdl_feedback_max_chars: int = 5000
    hdl_keep_judge_work: bool = False
    hdl_env_sh: Optional[str] = None
    hdl_simulate_tool_bonus: float = 0.03
    hdl_synthesize_tool_bonus: float = 0.02
    hdl_tool_bonus_min_final_score: float = 0.05
    hdl_reward_cap: float = 1.05
