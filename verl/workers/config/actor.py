...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        loss_mode (str): Loss function mode. Options include 'vanilla', 'turn_level_ppo', 'clip-cov',
            'kl-cov', 'gpg', 'simple_intentional_grpo', 'vanilla_adaptive_alpha_grpo',
            and 'vanilla_norm_matched_alpha_grpo'.
    ...
    intentional_eta: float = 1.0
    intentional_score_norm_eps: float = 1e-8
    intentional_clip_target: bool = True
    intentional_require_sum_pi_squared: bool = True
    intentional_alpha_min: float = 0.0
    intentional_alpha_max: Optional[float] = None
    state_predictive_use_update_sketch: bool = True
    state_predictive_normalize_features: bool = True
    state_predictive_min_segment_len: int = 2
    state_predictive_max_segment_len: int = 128
    state_predictive_dinkelbach_iters: int = 6
    state_predictive_tol: float = 1e-4
...
class McoreActorConfig(ActorConfig):
    ...
    calculate_update_sketch: bool = False
    update_sketch_dim: int = 64
    update_sketch_seed: int = 17

    def __post_init__(self):
        """Validate FSDP actor configuration parameters."""
        super().__post_init__()
        object.__setattr__(self.megatron, "calculate_sum_pi_squared", self.calculate_sum_pi_squared)
        object.__setattr__(self.megatron, "sum_pi_squared_checkpointing", self.sum_pi_squared_checkpointing)
        object.__setattr__(self.megatron, "calculate_update_sketch", self.calculate_update_sketch)
        object.__setattr__(self.megatron, "update_sketch_dim", self.update_sketch_dim)
        object.__setattr__(self.megatron, "update_sketch_seed", self.update_sketch_seed)
