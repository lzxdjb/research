...
from verl.utils.megatron.tensor_parallel import (
    ...
    vocab_parallel_score_sketch_from_logits,
...
def _hf_config_uses_vision_positions(hf_config: Any) -> bool:
    """Return True for VLM/Omni configs that calculate position ids internally."""
    if hasattr(hf_config, "vision_config"):
        return True
    model_type = str(getattr(hf_config, "model_type", "")).lower()
    architectures = [str(item).lower() for item in getattr(hf_config, "architectures", []) or []]
    if "omni" in model_type or any("omni" in item or "vl" in item for item in architectures):
        return True
    thinker_config = getattr(hf_config, "thinker_config", None)
    if isinstance(thinker_config, dict) and thinker_config.get("vision_config"):
        return True
    if hasattr(thinker_config, "vision_config"):
        return True
    return False
...
class MegatronEngineWithLMHead(MegatronEngine):
    ...
    def prepare_model_outputs(self, output: dict, data: TensorDict):
        ...
        calculate_sum_pi_squared = tu.get_non_tensor_data(data, key="calculate_sum_pi_squared", default=False)
        calculate_update_sketch = tu.get_non_tensor_data(data, key="calculate_update_sketch", default=False)
        ...
        if calculate_sum_pi_squared:
            model_output["sum_pi_squared"] = output["sum_pi_squared"]
        if calculate_update_sketch:
            model_output["update_sketch"] = output["update_sketch"]
    ...
    def forward_step(self, batch_iter: Iterator[TensorDict], model, postprocess_micro_batch_func):
        ...
        calculate_update_sketch = tu.get_non_tensor_data(batch, key="calculate_update_sketch", default=False)
        update_sketch_dim = int(tu.get_non_tensor_data(batch, key="update_sketch_dim", default=64))
        update_sketch_seed = int(tu.get_non_tensor_data(batch, key="update_sketch_seed", default=17))
        ...
        if use_fused_kernels:
            ...
            if use_fused_kernels and (calculate_sum_pi_squared or calculate_update_sketch):
                logger.warning_once(
                    "sum_pi_squared/update_sketch requires logits, so Megatron engine falls back to non-fused "
        ...
        if use_fused_kernels:
            ...
            def logits_processor(logits, label, temperature):
                ...
                if calculate_sum_pi_squared:
                    ret["sum_pi_squared"] = vocab_parallel_sum_pi_squared_from_logits(logits)
                if calculate_update_sketch:
                    ret["update_sketch"] = vocab_parallel_score_sketch_from_logits(
                        logits,
                        label,
                        sketch_dim=update_sketch_dim,
                        seed=update_sketch_seed,
                    )
            ...
            output = forward_fn(
                ...
                vision_model=_hf_config_uses_vision_positions(self.model_config.hf_config),
                pad_token_id=self.model_config.tokenizer.pad_token_id,
