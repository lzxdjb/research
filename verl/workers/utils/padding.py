...
def no_padding_2_padding(tensor: torch.Tensor, data: TensorDict) -> torch.Tensor:
    ...
    for resp_len, seq_offset in zip(response_lens, sequence_offsets, strict=True):
        pad_size = int(max_response_len - resp_len)
        # left-shift model output by one token for log_probs/values
        response = values[seq_offset - resp_len - 1 : seq_offset - 1]
        if values.dim() == 1:
            response = F.pad(response, (0, pad_size))
        else:
            pad_shape = (pad_size, *values.shape[1:])
            response = torch.cat(
                [response, torch.zeros(pad_shape, dtype=values.dtype, device=values.device)],
                dim=0,
            )
