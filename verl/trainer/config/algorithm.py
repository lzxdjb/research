...
class AlgoConfig(BaseConfig):
    ...
    # When enabled for GAE, broadcast one value advantage/return to all tokens in each assistant turn.
    turn_level_value: bool = False
    # Which token anchors each turn's value target: first, last, or mean.
    turn_level_value_anchor: str = "first"
    ...
    # Latent-factor GRPO: learns a small rollout-batch factorizer that
    # redistributes the prompt-group scalar GRPO advantage over response tokens.
    latent_factor_k: int = 8
    latent_factor_hidden_dim: int = 32
    latent_factor_aux_steps: int = 8
    latent_factor_lr: float = 1e-2
    latent_factor_tau: float = 1.0
    latent_factor_balance: bool = True
    latent_factor_balance_iters: int = 4
    latent_factor_preserve_scalar_mean: bool = True
    latent_factor_residual_correction: bool = True
    latent_factor_mix_with_vanilla: float = 0.0

    # Batch-optimal subspace GRPO: estimates a low-rank subspace of coherent
    # rollout-batch update proxies and downweights trajectories whose proxy
    # updates lie mostly outside that subspace.
    bos_grpo_k: int = 4
    bos_grpo_lambda: float = 1.0
    bos_grpo_weight_floor: float = 0.1
    bos_grpo_weight_power: float = 1.0
    bos_grpo_mix_with_vanilla: float = 0.0
    bos_grpo_positive_eigs_only: bool = True
    bos_grpo_fallback_to_vanilla: bool = True
    bos_grpo_include_feature_std: bool = True
    bos_grpo_normalize_features: bool = True
    bos_grpo_eps: float = 1e-6

    # Multi-domain BOS-GRPO: estimates one shared low-rank subspace from
    # domain-level proxy update means and downweights trajectories outside it.
    md_bos_grpo_k: int = 4
    md_bos_grpo_domain_lambda: float = 1.0
    md_bos_grpo_noise_lambda: float = 1.0
    md_bos_grpo_weight_floor: float = 0.1
    md_bos_grpo_weight_power: float = 1.0
    md_bos_grpo_mix_with_vanilla: float = 0.0
    md_bos_grpo_positive_eigs_only: bool = True
    md_bos_grpo_fallback_to_vanilla: bool = True
    md_bos_grpo_include_feature_std: bool = True
    md_bos_grpo_normalize_features: bool = True
    md_bos_grpo_eps: float = 1e-6
    md_bos_grpo_domain_key: str = "domain"
    md_bos_grpo_min_domain_count: int = 2

    # Shared-private multi-domain BOS-GRPO: estimates one shared low-rank
    # subspace plus a private low-rank residual subspace for each domain.
    sp_md_bos_grpo_shared_k: int = 4
    sp_md_bos_grpo_private_k: int = 2
    sp_md_bos_grpo_domain_lambda: float = 1.0
    sp_md_bos_grpo_shared_noise_lambda: float = 1.0
    sp_md_bos_grpo_private_noise_lambda: float = 1.0
    sp_md_bos_grpo_weight_floor: float = 0.3
    sp_md_bos_grpo_weight_power: float = 1.0
    sp_md_bos_grpo_mix_with_vanilla: float = 0.3
    sp_md_bos_grpo_positive_eigs_only: bool = True
    sp_md_bos_grpo_fallback_to_vanilla: bool = True
    sp_md_bos_grpo_include_feature_std: bool = True
    sp_md_bos_grpo_normalize_features: bool = True
    sp_md_bos_grpo_eps: float = 1e-6
    sp_md_bos_grpo_domain_key: str = "domain"
    sp_md_bos_grpo_min_domain_count: int = 2

    # SNR multi-domain GRPO: uses sketch-based update proxies to estimate
    # same-domain signal, cross-domain conflict, and within-domain instability.
    snr_md_grpo_include_feature_std: bool = False
    snr_md_grpo_normalize_features: bool = True
    snr_md_grpo_eps: float = 1e-6
    snr_md_grpo_domain_key: str = "domain"
    snr_md_grpo_min_domain_count: int = 2

    # Coefficient for MI proxy reward shaping. When > 0, the trainer computes an
    # in-batch contrastive prompt-retrieval score and adds it to the terminal reward.
    mi_reward_coef: float = 0.0

    # Number of negative prompts to compare against for each sample when building the
    # prompt-retrieval proxy. Set to 0 to use all eligible negatives in the batch.
    mi_reward_num_negatives: int = 3

    # Listwise Policy Optimization target settings.
    # lpo_projection="forward" uses bounded coefficients K * (w* - uniform).
    # lpo_projection="reverse" uses the reverse-KL on-policy coefficient.
    lpo_projection: str = "forward"
    lpo_tau: float = 1.0
    lpo_eps: float = 1e-8
    # Adaptive LPO chooses a = 1/tau by maximizing reward gain times relative ESS.
    ...
    # Exponential moving average coefficient used to stabilize MI z-score normalization.
    mi_reward_ema_alpha: float = 0.9

    # Numerical stabilizer for MI z-score normalization.
    mi_reward_zscore_eps: float = 1e-3
