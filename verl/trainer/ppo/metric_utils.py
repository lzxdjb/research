...
def _is_numeric_metric_value(value: Any) -> bool:
    """Return True for scalar numeric values that are safe to aggregate."""
...
def _numeric_metric_values(values: Any) -> list[float]:
    """Return finite scalar numeric values from a reward-extra value sequence."""
...
def compute_reward_extra_metrics(
    ...
    *,
    ...
    """Aggregate numeric reward-extra fields for training logs.

    Reward functions can return diagnostic fields next to ``score``. This turns
    numeric fields into mean metrics so they are visible in console/WandB during
    training, not only in validation artifacts.
    """

    metrics: dict[str, float] = {}
    for key, values in (reward_extra_infos_dict or {}).items():
        numeric_values = _numeric_metric_values(values)
        if not numeric_values:
            continue
        metrics[f"{prefix}/{key}/mean"] = float(np.mean(numeric_values))

    if f"{prefix}/bank_submission_success/mean" in metrics:
        metrics["reward/submission_ratio"] = metrics[f"{prefix}/bank_submission_success/mean"]
    if f"{prefix}/bank_procedure_reward/mean" in metrics:
        metrics["reward/procedure_reward_mean"] = metrics[f"{prefix}/bank_procedure_reward/mean"]
    if f"{prefix}/bank_final_reward/mean" in metrics:
        metrics["reward/final_reward_mean"] = metrics[f"{prefix}/bank_final_reward/mean"]
    if f"{prefix}/bank_completion_ratio/mean" in metrics:
        metrics["reward/bank_completion_ratio_mean"] = metrics[f"{prefix}/bank_completion_ratio/mean"]
    return metrics
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                # skip empty, string, list, dict, and other non-scalar diagnostic values
