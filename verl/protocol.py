...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        # Non-tensor keys can differ across shards in multi-turn rollouts: a
        # trajectory may terminate before a tool/reward path adds optional
        # metadata such as scenario_json.  Keep the union of keys and pad
        # missing shard entries so every non-tensor column stays aligned with
        # the concatenated tensor batch.
        non_tensor_dicts = [d.non_tensor_batch or {} for d in data]
        non_tensor_batch = {}
        all_non_tensor_keys = set().union(*(nt.keys() for nt in non_tensor_dicts)) if non_tensor_dicts else set()
        for key in all_non_tensor_keys:
            template = next((nt[key] for nt in non_tensor_dicts if key in nt), None)
            if template is not None and not isinstance(template, np.ndarray):
                template = np.array(template, dtype=object)
            tail_shape = tuple(getattr(template, "shape", ())[1:])
            parts = []
            for d, nt in zip(data, non_tensor_dicts, strict=False):
                if key in nt:
                    arr = nt[key]
                    ...
                    missing = np.empty((len(d),) + tail_shape, dtype=object)
                    missing.fill(None)
