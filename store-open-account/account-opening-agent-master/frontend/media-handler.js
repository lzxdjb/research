...
class MediaHandler {
  constructor() {
    ...
    this.lastSentFrame = null; // last frame base64 sent to Gemini
  }
  ...
  async startVideo(videoElement, onFrame) {
    try {
      ...
      // Populate lastSentFrame as soon as the video is ready to play,
      // without waiting for the first 1-second interval tick.
  ...
  captureFrame(videoElement, onFrame) {
    ...
    const base64 = this.videoCanvas.toDataURL("image/png").split(",")[1];
    this.lastSentFrame = base64; // cache the frame Gemini actually sees
    onFrame(base64);
  }

  captureFrameAsBlob(videoElement) {
  ...
  async uploadBlob(blob, filename = "capture.jpg") {
    // Returns {fileData, filename} for caller to upload via WebSocket
