...
LANGCHAIN_SERVER_URLS = [
    os.getenv("IWENCAI_CHAIN_URL", "http://122.224.107.233:880/iwencai/dialog/chain/execute"),
    "http://190.92.231.77:880/iwencai/dialog/chain/execute",
]


def request_chain(json: dict, timeout=10):
    trace_id = str(uuid.uuid4())
    user_id = f"user_{uuid.uuid4().hex[:8]}"
    session_id = f"session_{uuid.uuid4().hex}"
    # print(f"Requesting chain with trace_id: {trace_id}, user_id: {user_id}, session_id: {session_id}")
    ...
    last_exception = None
    for langchain_server_url in dict.fromkeys(LANGCHAIN_SERVER_URLS):
        try:
            return requests.post(langchain_server_url, headers=langchain_headers, json=json, timeout=timeout)
        except requests.exceptions.RequestException as exc:
            last_exception = exc
            print(f"Request to {langchain_server_url} failed: {exc}")
...
if __name__ == '__main__':
    ...
    res = get_tools_results([
                    ...
                    # {'name': 'FinQuery', 'input': '上证指数最近一周的行情数据'}
                    # {"name": "VisitWeb", "input": "https://news.10jqka.com.cn/20251030/c672126342.shtml"},
                    # {"name":"FinQuery", "input": "最新发布的中国国内生产总值(GDP)同比增长率"},
