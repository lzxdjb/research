...
from prompt import DEEP_SYS_PROMPT, SIMPLE_SYS_PROMPT, SUMMARY_PROMPT
import argparse
...
def process_simple_record(record, simple_sys_prompt, summary_sys_prompt, fout_simple_plan, fout_simple_summary, online_client, local_client, input_images_dir, output_images_dir):
    stage = 'simple'
    ...
    messages= [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": simple_sys_prompt + query
                    }]
                    }
            ]
    messages[0]['content'].extend(img_message[0])
    planning_results = local_client.image2text({'messages': messages}) # 初步规划
    ...
    while check_simple_planning_result(best_result_str) and turns <=10: # 继续规划
        ...
        planning_results = local_client.image2text({'messages': messages})
        if planning_results == 'error: ' or planning_results is None or 'Traceback' in planning_results:
            print("Error: planning_results are None")
            return
        messages, best_result = score_results(messages, planning_results, question, type, stage, online_client, fout_simple_plan)
    ...
    summary_messages= [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": summary_sys_prompt + f'### 背景及参考消息\n<背景>\n\n<参考>\n'
                    }]
                    }
            ]
    ...
    summary_results = local_client.image2text({'messages': summary_messages}) # 普通总结
...
def process_deep_record(record, deep_sys_prompt, summary_sys_prompt, simple_information, fout_deep_plan, fout_deep_summary, online_client, local_client, input_images_dir, output_image_dir, idx, full_tools_results):
    stage = 'deep'
    question = record['query']
    type = record['type']
    time = record['time']
    ...
    messages= [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": deep_sys_prompt + query
                    }]
                    }
            ]
    messages[0]['content'].extend(img_message[0])
    messages[0]['content'].extend(simple_information)
    planning_results = local_client.image2text({'messages': messages}) # 深度规划
    if planning_results == 'error: ' or planning_results is None or 'Traceback' in planning_results:
        print("Error: planning_results are None")
        return
...
if __name__ == "__main__":
    ...
    ROOT_PATH = r'D:\data\code\业务\simulation\evaluation\2508\04'
    model_name = 'debug'
    os.makedirs(fr"{ROOT_PATH}\data\{model_name}", exist_ok=True)
    parser.add_argument('--input', type=str, 
                        default=fr"{ROOT_PATH}\data\debug.jsonl",
    ...
    parser.add_argument('--deep_plan_output', type=str, 
                        default=fr"{ROOT_PATH}\data\{model_name}\deep_plan_output.jsonl",
                        help='Output JSONL file path')
    parser.add_argument('--deep_summary_output', type=str, 
                        default=fr"{ROOT_PATH}\data\{model_name}\deep_summary_output.jsonl",
                        help='Output JSONL file path')
    parser.add_argument('--input_images_dir', type=str,
                        default=fr"{ROOT_PATH}\input_images",
                        help='Input images directory path')
    parser.add_argument('--output_images_dir', type=str,
                        default=fr"{ROOT_PATH}\data\{model_name}\output_images",
    ...
    sys_prompt = DEEP_SYS_PROMPT
    simple_sys_prompt = SIMPLE_SYS_PROMPT
    summary_sys_prompt = SUMMARY_PROMPT
    online_client = GEMINIClient()
    local_client = LocalModelClient(args.model)
        ...
        for line in fin:
            if idx > 6:
                ...
                try:
                    ...
                    simple_information, info_id, full_tools_results = process_simple_record(record, simple_sys_prompt, summary_sys_prompt, fout_simple_plan, fout_simple_summary, online_client, local_client, args.input_images_dir, args.output_images_dir)
                    process_deep_record(record, sys_prompt, summary_sys_prompt, simple_information, fout_deep_plan, fout_deep_summary, online_client, local_client, args.input_images_dir, args.output_images_dir, info_id, full_tools_results)
