...
def process_simple_record(record, simple_plan_prompt, simple_summary_prompt, fout_simple_plan, fout_simple_summary, online_client, local_client, input_images_dir, output_images_dir):
    ...
    # planning_results = local_client.image2text_N({'messages': messages}, num_samples=5) # 初步规划
...
def process_deep_record(record, deep_plan_prompt, deep_summary_prompt, simple_information, fout_deep_plan, fout_deep_summary, online_client, local_client, input_images_dir, output_image_dir, idx, full_tools_results):
    stage = 'deep'
    question = record['query']
    type = record['type']
    time = record['time']
    ...
    # planning_results = local_client.image2text_N({'messages': messages}, num_samples=5) # 深度规划
    planning_results = online_client.image2text_N({'messages': messages}, num_samples=5) # 深度规划
    if planning_results == 'error: ' or planning_results is None or 'Traceback' in planning_results:
        print("Error: planning_results are None")
        return
...
if __name__ == "__main__":
    ...
    ROOT_PATH = r'D:\data\code\业务\simulation\synthesis\dpo\2508\15'
    model_name = 'draw'
        ...
        for line in fin:
            if idx > 182:
                ...
                try:
                    ...
                    process_deep_record(record, deep_plan_prompt, deep_summary_prompt, simple_information, fout_deep_plan, fout_deep_summary, online_client, local_client, args.input_images_dir, args.output_images_dir, info_id, full_tools_results)
