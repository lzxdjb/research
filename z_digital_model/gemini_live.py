...
def _load_system_prompt() -> str:
    """Load the onboarding system prompt beside this module."""
    prompt_paths = [
        Path(__file__).with_name("gemini_system_prompt.md"),
        Path(__file__).parent / "agent" / "gemini_system_prompt.md",
    ]
    for prompt_path in prompt_paths:
        if prompt_path.exists():
            return prompt_path.read_text(encoding="utf-8")
    logger.warning(f"System prompt not found at any of {prompt_paths}, using fallback")
...
class GeminiLive:
    ...
    async def start_session(self, audio_input_queue, video_input_queue, text_input_queue, audio_output_callback, audio_interrupt_callback=None):
        config = types.LiveConnectConfig(
            ...
            system_instruction=types.Content(parts=[types.Part(text=_load_system_prompt())]),
