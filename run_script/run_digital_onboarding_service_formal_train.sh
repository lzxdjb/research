...
BRANCH_MODE="${BRANCH_MODE:-${DIGITAL_ONBOARDING_BRANCH_MODE:-us_market}}"
...
if [[ -z "$CUSTOM_TRAIN_FILE" && "${BUILD_DATA_IF_STALE:-1}" == "1" ]]; then
  NEED_BUILD_DATA=0
  if [[ ! -f "$DEFAULT_TRAIN_FILE" || ! -f "$DEFAULT_VAL_FILE" ]]; then
    NEED_BUILD_DATA=1
  else
    TRAIN_ROWS="$(parquet_row_count "$DEFAULT_TRAIN_FILE")"
    VAL_ROWS="$(parquet_row_count "$DEFAULT_VAL_FILE")"
    if [[ "$TRAIN_ROWS" != "$TRAIN_SIZE" || "$VAL_ROWS" != "$VAL_SIZE" ]]; then
      echo "Dataset row count mismatch: train=$TRAIN_ROWS expected=$TRAIN_SIZE, val=$VAL_ROWS expected=$VAL_SIZE"
      NEED_BUILD_DATA=1
    fi
  fi
  if [[ "$NEED_BUILD_DATA" == "0" ]]; then
    for src in \
      recipe/digital_onboarding/service_system_prompt.md \
      recipe/digital_onboarding/scenario.py \
      recipe/digital_onboarding/tools.py \
      recipe/digital_onboarding/real_bank.py \
      recipe/digital_onboarding/config/tool_config.yaml \
      recipe/digital_onboarding/scripts/build_data.py; do
      if [[ "$src" -nt "$DEFAULT_TRAIN_FILE" ]]; then
        NEED_BUILD_DATA=1
        break
      fi
    done
  fi
  if [[ "$NEED_BUILD_DATA" == "1" ]]; then
    echo "Building digital onboarding dataset at $DATA_DIR"
    python3 -m recipe.digital_onboarding.scripts.build_data \
      ...
      --tool-backend "$DIGITAL_ONBOARDING_TOOL_BACKEND"
  fi
fi
...
echo "Digital onboarding branch mode: $BRANCH_MODE"
echo "Digital onboarding tool backend: $DIGITAL_ONBOARDING_TOOL_BACKEND"
echo "Training CUDA_VISIBLE_DEVICES: ${CUDA_VISIBLE_DEVICES:-ALL_VISIBLE_GPUS}"
echo "Training GPU count: $VISIBLE_GPU_COUNT"
echo "Training max samples: $TRAIN_SIZE"
...
export DIGITAL_ONBOARDING_PROCEDURE_REWARD_ENABLED="${DIGITAL_ONBOARDING_PROCEDURE_REWARD_ENABLED:-1}"
export DIGITAL_ONBOARDING_PROCEDURE_REWARD_WEIGHT="${DIGITAL_ONBOARDING_PROCEDURE_REWARD_WEIGHT:-0.15}"
export DIGITAL_ONBOARDING_FINAL_SUBMIT_REWARD="${DIGITAL_ONBOARDING_FINAL_SUBMIT_REWARD:-1.0}"
...
echo "Procedure reward enabled: $DIGITAL_ONBOARDING_PROCEDURE_REWARD_ENABLED"
echo "Procedure reward weight: $DIGITAL_ONBOARDING_PROCEDURE_REWARD_WEIGHT"
echo "Final submit reward: $DIGITAL_ONBOARDING_FINAL_SUBMIT_REWARD"
...
python3 -m verl.trainer.main_ppo \
  ...
  data.train_max_samples="$TRAIN_SIZE" \
