...
JOB_NAME=${JOB_NAME:-state_evidence_joint_grpo_9B}
...
STATE_EVIDENCE_MAX_SEGMENT_LEN=${STATE_EVIDENCE_MAX_SEGMENT_LEN:-512}
STATE_EVIDENCE_MIN_SEGMENT_LEN=${STATE_EVIDENCE_MIN_SEGMENT_LEN:-2}
STATE_EVIDENCE_SEGMENT_BACKEND=${STATE_EVIDENCE_SEGMENT_BACKEND:-torch}
STATE_EVIDENCE_PRECOMPUTE_STATE_INDEX=${STATE_EVIDENCE_PRECOMPUTE_STATE_INDEX:-True}
STATE_EVIDENCE_USE_UPDATE_SKETCH=${STATE_EVIDENCE_USE_UPDATE_SKETCH:-True}
STATE_EVIDENCE_NORMALIZE_FEATURES=${STATE_EVIDENCE_NORMALIZE_FEATURES:-True}
# Negative values enable per-trajectory empirical-Bayes estimation.
STATE_EVIDENCE_NOISE_VAR=${STATE_EVIDENCE_NOISE_VAR:--1.0}
STATE_EVIDENCE_PRIOR_VAR=${STATE_EVIDENCE_PRIOR_VAR:--1.0}
STATE_JOINT_MAX_ABS_LOG_RATIO=${STATE_JOINT_MAX_ABS_LOG_RATIO:-20.0}
CALCULATE_UPDATE_SKETCH=${CALCULATE_UPDATE_SKETCH:-$STATE_EVIDENCE_USE_UPDATE_SKETCH}
...
python3 -m verl.trainer.main_ppo --config-path=config \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=state_evidence_joint_grpo \
    +actor_rollout_ref.actor.policy_loss.state_predictive_objective=evidence \
    +actor_rollout_ref.actor.policy_loss.state_predictive_use_update_sketch=$STATE_EVIDENCE_USE_UPDATE_SKETCH \
    +actor_rollout_ref.actor.policy_loss.state_predictive_normalize_features=$STATE_EVIDENCE_NORMALIZE_FEATURES \
    +actor_rollout_ref.actor.policy_loss.state_predictive_min_segment_len=$STATE_EVIDENCE_MIN_SEGMENT_LEN \
    +actor_rollout_ref.actor.policy_loss.state_predictive_max_segment_len=$STATE_EVIDENCE_MAX_SEGMENT_LEN \
    +actor_rollout_ref.actor.policy_loss.state_predictive_segment_backend=$STATE_EVIDENCE_SEGMENT_BACKEND \
    +actor_rollout_ref.actor.policy_loss.state_predictive_precompute_state_index=$STATE_EVIDENCE_PRECOMPUTE_STATE_INDEX \
    +actor_rollout_ref.actor.policy_loss.state_evidence_noise_var=$STATE_EVIDENCE_NOISE_VAR \
    +actor_rollout_ref.actor.policy_loss.state_evidence_prior_var=$STATE_EVIDENCE_PRIOR_VAR \
    +actor_rollout_ref.actor.policy_loss.state_joint_max_abs_log_ratio=$STATE_JOINT_MAX_ABS_LOG_RATIO \
