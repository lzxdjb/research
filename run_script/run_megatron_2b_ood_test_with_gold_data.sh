...
# export ALFWORLD_SERVER_HOST="10.248.100.212"   # e.g. 192.168.1.42
# export ALFWORLD_BASE_PORT=8800
# export ALFWORLD_NUM_SERVERS=64

# # Verify connectivity before training
# echo "Checking connectivity to remote servers..."
# READY=0
# for i in $(seq 0 $((ALFWORLD_NUM_SERVERS - 1))); do
#     PORT=$((ALFWORLD_BASE_PORT + i))
#     if curl -sf http://${ALFWORLD_SERVER_HOST}:${PORT}/health > /dev/null 2>&1; then
#         READY=$((READY + 1))
#     fi
# done
# echo "$READY / $ALFWORLD_NUM_SERVERS servers reachable."

# if [ $READY -lt $ALFWORLD_NUM_SERVERS ]; then
...
train_path=/cpfs01/nlp/leizhengxing/stock-rl/data/mixed_new_train_2_600_gold_train/train.parquet
test_path=/cpfs01/nlp/leizhengxing/stock-rl/data/new_train/ood_dataset.parquet
