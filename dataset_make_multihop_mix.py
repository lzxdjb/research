...
SYSTEM_PROMPT = (
    ...
    "Do not include extra explanation inside the FINISHED block."
)
...
def load_source(
    ...
    exclude_qids: set of question IDs to skip (used to keep test isolated from train).
    """
    iterate = _ITERS[name]
    rows = []
    skipped = 0
...
def main():
    ...
    for name, path_str, stem in test_sources:
        if path_str is None:
            ...
            continue
        rows = load_source(name, Path(path_str), max_samples=args.max_test_per_source)
